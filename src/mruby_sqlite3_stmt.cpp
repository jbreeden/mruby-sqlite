...
mrb_SQLite_Sqlite3Stmt_initialize(mrb_state* mrb, mrb_value self) {
  struct sqlite3_stmt* native_object = (struct sqlite3_stmt*)malloc(sizeof(struct sqlite3_stmt));
  mruby_gift_struct sqlite3_stmt_data_ptr(self, native_object);
  return self;
}
#endif
...
void mrb_SQLite_Sqlite3Stmt_init(mrb_state* mrb) {
  RClass* Sqlite3Stmt_class = mrb_define_class_under(mrb, SQLite_module(mrb), "Sqlite3Stmt", mrb->object_class);
